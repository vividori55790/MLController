...
% - Current Version: v4.3.0_Final (2026-06-26)
...
% * v4.2.0_Final (2026-06-26) - Developer: Gemini AI
%   - Added Simscape conduction losses and fixed stability check bug.
% * v4.1.0_Final (2026-06-26) - Developer: Gemini AI
%   - Consolidated Particle Swarm Optimization (PSO) loops directly within the script.
% ======================================================================
...
Vin_data(t_vec >= 0.04) = 18;               % 40ms: 입력 서지 (+50% Surge)
rng(42); Vin_data = Vin_data + 0.3 * randn(N_pts, 1); % 고주파 노이즈 합성
Vref_data = Vref_val * ones(N_pts, 1);

% From Workspace 블록용 Matrix / Timeseries 데이터 공급
Vin = [t_vec, Vin_data];
R = [t_vec, R_data];
L = [t_vec, L_val * ones(N_pts, 1)];
C = [t_vec, C_val * ones(N_pts, 1)];
Vref = [t_vec, Vref_data];
Vs = Vin;

Vin_ts = timeseries(Vin_data, t_vec);
R_ts = timeseries(R_data, t_vec);
L_ts = timeseries(L_val * ones(N_pts, 1), t_vec);
C_ts = timeseries(C_val * ones(N_pts, 1), t_vec);
...
% 최악의 조건(R = 2 Ohm) 베이스라인 공칭 매트릭스 도출
R_worst = 2.0;
...
% (2) 3P2Z 제어기 이산 안정성 마진 가드레일 최적화 (2변수: f_co, PM_target)
...
lb_lqr = [10.0, 1e4, 10.0, 0.001]; ub_lqr = [1000.0, 1e7, 500.0, 1.0];
...
if exist(simulink_model_name, 'file') == 4
    ...
    try
        ...
        % 3P2Z 및 ML 이산 전달 함수 블록의 샘플 타임을 Ts로 강제 설정하여 연속 도메인 상속 버그 차단
        fprintf('- 이산 전달함수 블록 샘플 타임 강제 매칭 적용 (SampleTime = Ts)...\n');
        set_param([simulink_model_name '/Discrete Transfer Fcn'], 'SampleTime', 'Ts');
        set_param([simulink_model_name '/Discrete Transfer Fcn1'], 'SampleTime', 'Ts');
...
if simulink_running
    ...
    for k = 1:N_pts
        ...
        % --- [1] PI Control Loop ---
        R_sd_pi = duty_pi * R_sw + (1 - duty_pi) * Ron;
        v_sw_pi = duty_pi * V_in_k - (1 - duty_pi) * Vf - x_pi(1) * R_sd_pi;
        V_out_pi = C_k * x_pi + D_k * v_sw_pi;
        ...
        % --- [2] 3P2Z Control Loop (1-sample delay of strictly proper TF block) ---
        R_sd_3p = duty_3p2z * R_sw + (1 - duty_3p2z) * Ron;
        v_sw_3p2z = duty_3p2z * V_in_k - (1 - duty_3p2z) * Vf - x_3p2z(1) * R_sd_3p;
        V_out_3p2z = C_k * x_3p2z + D_k * v_sw_3p2z;
        err_3p2z = Vref_val - V_out_3p2z; 
        err_hist_3p2z = [err_3p2z; err_hist_3p2z(1:3)];
        duty_3p2z_raw = n1*err_hist_3p2z(2) + n2*err_hist_3p2z(3) + n3*err_hist_3p2z(4) ...
                        - d2*u_hist_3p2z(1) - d3*u_hist_3p2z(2) - d4*u_hist_3p2z(3);
        duty_3p2z = max(0.01, min(0.95, duty_3p2z_raw)); u_hist_3p2z = [duty_3p2z; u_hist_3p2z(1:2)];
        
        % --- [3] ML High-Order Loop ---
        R_sd_ml = duty_ml * R_sw + (1 - duty_ml) * Ron;
        v_sw_ml = duty_ml * V_in_k - (1 - duty_ml) * Vf - x_ml(1) * R_sd_ml;
        V_out_ml = C_k * x_ml + D_k * v_sw_ml;
        err_ml = Vref_val - V_out_ml; err_hist_ml = [err_ml; err_hist_ml(1:5)];
        ...
        duty_ml = max(0.01, min(0.95, duty_ml_raw)); u_hist_ml = [duty_ml; u_hist_ml(1:4)];

        % --- [4] Modern LQR Servo Loop ---
        R_sd_lqr = duty_lqr * R_sw + (1 - duty_lqr) * Ron;
        v_sw_lqr = duty_lqr * V_in_k - (1 - duty_lqr) * Vf - x_lqr(1) * R_sd_lqr;
        V_out_lqr = C_k * x_lqr + D_k * v_sw_lqr;
...
%% 4. 정량적 성능 지표 분석 및 출력
get_t_s = @(v, t, target) feval(@(idx) feval(@(is_empty) is_empty*0 + (~is_empty)*feval(@(v_val) v_val(1), [t(idx); 0]), isempty(idx)), find(abs(movmean(v, 15) - target) > 0.02 * target, 1, 'last'));
...
% Simulink 모델 안전 종료 (저장 없이 닫아 대기 프롬프트 차단)
if exist('simulink_model_name', 'var') && bdIsLoaded(simulink_model_name)
    close_system(simulink_model_name, 0);
end
...
function gbest = pso_optimize(cost_fn, n_vars, lb, ub, max_iter, pop_size)
    w = 0.6; c1 = 1.5; c2 = 1.5;
    pos = lb + (ub - lb) .* rand(pop_size, n_vars); vel = zeros(pop_size, n_vars);
    pbest = pos; pbest_cost = inf(pop_size, 1); gbest = zeros(1, n_vars); gbest_cost = inf;
    for iter = 1:max_iter
        for i = 1:pop_size
            cost = cost_fn(pos(i, :));
            if cost < pbest_cost(i), pbest(i, :) = pos(i, :); pbest_cost(i) = cost; end
            if cost < gbest_cost, gbest = pos(i, :); gbest_cost = cost; end
        end
        vel = w * vel + c1 * rand(pop_size, n_vars) .* (pbest - pos) + c2 * rand(pop_size, n_vars) .* (gbest - pos);
        pos = pos + vel; pos = max(lb, min(ub, pos));
    end
end

function x_next = rk4_step(A, B, x, u, dt)
    k1 = A * x + B * u; k2 = A * (x + 0.5 * dt * k1) + B * u;
    k3 = A * (x + 0.5 * dt * k2) + B * u; k4 = A * (x + dt * k3) + B * u;
...
function cost = evaluate_pi_cost(KP, KI, L_val, C_val, G_L, R_C, Vin_data, R_data, Vref_val, T_s, t_vec, N_pts)
    ...
    for k = 1:N_pts
        ...
        R_sd = duty * R_sw + (1 - duty) * Ron;
        v_sw = duty * V_in_k - (1 - duty) * Vf - x(1) * R_sd;
        V_out = C_k * x + D_k * v_sw; V_out_log(k) = V_out;
        
        err = Vref_val - V_out; error_int = error_int + err * T_s;
        duty_raw = KP * err + KI * error_int; duty = max(0.01, min(0.95, duty_raw));
...
function cost = evaluate_3p2z_cost(p, A_nom_c, B_nom_c, C_nom_c, D_nom_c, L_val, C_val, G_L, R_C, Vin_data, R_data, Vref_val, T_s, t_vec, N_pts)
    f_co = p(1); PM_target = p(2); w_co = 2 * pi * f_co; s_co = 1i * w_co;
    G_co = C_nom_c * ((s_co * eye(2) - A_nom_c) \ (B_nom_c * 12.0)) + D_nom_c;
    boost = PM_target - 90 - rad2deg(angle(G_co));
    if boost < 2 || boost > 175, cost = 1e8; return; end
    k_factor = tan(deg2rad(45 + boost / 4)); w_z = w_co / k_factor; w_p = w_co * k_factor;
    K_c = 1 / (abs(G_co) * ((w_co^2 + w_z^2) / (w_co * (w_co^2 + w_p^2))));
    A_p = 1 + w_p * T_s; A_z = 1 + w_z * T_s; d_raw1 = A_p^2;
    n1 = (K_c * T_s * A_z^2) / d_raw1; n2 = (-2 * K_c * T_s * A_z) / d_raw1; n3 = (K_c * T_s) / d_raw1;
    d2 = -(A_p^2 + 2*A_p) / d_raw1; d3 = (2*A_p + 1) / d_raw1; d4 = -1 / d_raw1;
    
    % [가드레일 패널티] 이산 극점 변형 안정 영역 검증 (z=1 적분극을 제외한 극점만 단위 원 내에 존재해야 함)
    r_3p = roots([1, d2, d3, d4]);
    r_3p_non_int = r_3p(abs(r_3p - 1) > 1e-5);
    if any(abs(r_3p_non_int) >= 0.98), cost = 1e12; return; end
    
    x = [0;0]; u_hist = zeros(3,1); err_hist = zeros(4,1); duty = Vref_val/Vin_data(1); V_out_log = zeros(N_pts, 1);
    R_sw = 0.01; Ron = 0.3; Vf = 0.6;
    for k = 1:N_pts
        V_in_k = Vin_data(k); R_k = R_data(k); theta_k = G_L * R_k * R_C + R_k + R_C;
        A_k = [ -R_k * R_C / (L_val * theta_k), -R_k / (L_val * theta_k); R_k / (C_val * theta_k), -(R_k * G_L + 1) / (C_val * theta_k) ];
        ...
        err = Vref_val - V_out; 
        err_hist = [err; err_hist(1:3)];
        duty_raw = n1*err_hist(2) + n2*err_hist(3) + n3*err_hist(4) ...
                 - d2*u_hist(1) - d3*u_hist(2) - d4*u_hist(3);
        duty = max(0.01, min(0.95, duty_raw)); u_hist = [duty; u_hist(1:2)];
        x = rk4_step(A_k, B_k, x, v_sw, T_s);
    end
...
function cost = evaluate_ml_ml_cost(p, L_val, C_val, G_L, R_C, Vin_data, R_data, Vref_val, T_s, t_vec, N_pts)
    ...
    % [가드레일 패널티] 전체 고차 분모 분해 검증 추가 (z=1 적분극을 제외한 den_z2 극점만 검사)
    if any(abs(roots(den_z2)) >= 0.98), cost = 1e12; return; end
    ...
    get_t_s = @(v_seg, t_seg, target) feval(@(idx) feval(@(is_empty) is_empty*0 + (~is_empty)*feval(@(v_val) v_val(1), [t_seg(idx); 0]), isempty(idx)), find(abs(movmean(v_seg, 15) - target) > 0.02 * target, 1, 'last'));
